...
class ModelOneAsset:
    ...
    # initial constructor
    def __init__(self, xyInit, muScal, sigScal, T, theta, sigma, lamb, eta, gamma, kappa, rho):
    ...
    def fDW( self, t, x, u, Du, D2u):  
        ...
        + 0.5 * tf.einsum('j,ij->i',tf.constant(self.gamma**2, dtype= tf.float32), hessy) \
        - tf.einsum('ij,ij->i',tf.einsum('j,ij->ij',tf.constant(self.rho*self.gamma*self.lamb, dtype = tf.float32), y), tf.einsum('ij,i->ij' ,tf.einsum('i,ij->ij', Du[:,0],D2u[:,0,1:]), tf.math.reciprocal(D2u[:,0,0])))\
        - 0.5 * tf.einsum('j,ij->i',tf.constant(self.rho**2*self.gamma**2, dtype = tf.float32), tf.einsum('ij,i->ij' ,tf.einsum('ij,ij->ij', D2u[:,0,1:],D2u[:,0,1:]), tf.math.reciprocal(D2u[:,0,0])))\
        -  tf.einsum('j,ij->i',tf.constant(self.muScal, dtype= tf.float32), Du)
    ...
    def fDWNumpy( self, t, x, u, Du, D2u):
        ...
        - np.einsum('ij,ij->i',np.einsum('j,ij->ij',self.rho*self.gamma*self.lamb, y), np.einsum('ij,i->ij' ,np.einsum('i,ij->ij', Du[:,0],D2u[:,0,1:]), 1/D2u[:,0,0]))\
        - 0.5 * np.einsum('j,ij->i',self.rho**2*self.gamma**2, np.einsum('ij,i->ij' ,np.einsum('ij,ij->ij', D2u[:,0,1:],D2u[:,0,1:]), 1/D2u[:,0,0]))\
    ...
    def w(self, t,y):
        ...
        return np.exp(-1.0*  np.einsum('jk,ij->i',self.phi(np.array([t])), y**2/2) - np.einsum('jk,ij->i',self.psi(np.array([t])),y) - chi)
    ...
    def getValuesExt(self, t):
        ...
        return np.tile(np.expand_dims(self.Init, axis=0),[2,1]) + \
            t*np.einsum('j,i->ij',self.muScal,np.ones([2])) + \
    ...
    def getValues(self, istep , TStep, xInit ,batchSize):
        return np.tile(np.expand_dims(xInit, axis=0),[batchSize,1]) + \
            TStep*np.einsum('j,i->ij',self.muScal,np.ones([batchSize]))*istep + \
