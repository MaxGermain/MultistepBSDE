...
class FeedForwardUZ:
    ...
    def createNetwork(self, X,iStep, renormalizeFactor):
        with tf.compat.v1.variable_scope("NetWorkUZ"+str(iStep) , reuse=tf.compat.v1.AUTO_REUSE):
    ...
    def createNetworkZ(self, X,iStep, renormalizeFactor):
        with tf.compat.v1.variable_scope("NetWorkZ"+str(iStep) , reuse=tf.compat.v1.AUTO_REUSE):
            fPrev= fc(X, int(self.layerSize[0]), scope='enc_fc1', activation_fn=self.activation)
            for i in np.arange(len(self.layerSize)-1):
                scopeName='enc_fc'+str(i+2)
                f = fc(fPrev,int(self.layerSize[i+1]), scope=scopeName, activation_fn=self.activation)
                fPrev = f
            UZ  = fc(fPrev,self.d, scope='UZ',activation_fn= None)
        return UZ
    ...
    def createNetworkNotTrainable(self, X,  iStep, weightInit, biasInit):
        with tf.compat.v1.variable_scope("NetWorkUZNonTrain"+"_"+str(iStep) , reuse= False):
            cMinW =0
            cMinB= 0
            ...
            for i in np.arange(len(self.layerSize)-1):
                ...
                cMinB+= self.layerSize[i+1]
                fPrev = f
            UDU  = fc(fPrev,self.d+1, scope='UZ',activation_fn= None, weights_initializer=tf.constant_initializer(np.reshape(weightInit[cMinW:cMinW+self.layerSize[len(self.layerSize)-1]*(self.d+1)],[self.layerSize[len(self.layerSize)-1],self.d+1])), biases_initializer= tf.constant_initializer(biasInit[cMinB:cMinB+(self.d+1)]), trainable= False)
        return  UDU[:,0], UDU[:,1:]    
    ...
    def getBackWeightAndBias(self,iStep):
        ...
        with tf.compat.v1.variable_scope("NetWorkUZ"+str(iStep), reuse=tf.compat.v1.AUTO_REUSE):
            ...
            Weights.append(tf.compat.v1.get_variable("UZ/weights",[self.layerSize[len(self.layerSize)-1],self.d+1]))
            Bias.append(tf.compat.v1.get_variable("UZ/biases",[self.d+1]))
        return Weights, Bias
