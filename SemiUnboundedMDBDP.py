...
d = 1
xInit= 0.5 * np.ones(d,dtype=np.float32) 
# nb neuron
nbNeuron =  10 + d
...
# create the model
model = mod.SemiUnbounded(sigScal, T, xInit)
...
for indt  in ndt:
    ...
    baseFile = "UnBoundedMDBDP"+str(d)+"nbNeur"+str(layerSize[0])+"nbHL"+str(len(layerSize))+"ndt"+str(indt)+"Sig"+str(int(100.*sig))
    ...
    for i in range(nTest):
        # train
        t0 = time.time()
        Y0, Z0    = resol.BuildAndtrainML( batchSize, batchSizeVal, num_epochExtNoLast=num_epochExtNoLast, num_epochExtLast= num_epochExtLast ,num_epoch=num_epoch,  nbOuterLearning=nbOuterLearning, thePlot= thePlot , baseFile = baseFile, cut = False, saveDir= "save/")
        t1 = time.time()
        print(" NBSTEP", indt, " EstimMC Val is " , Y0,  " REAL IS ",  model.SolPoint(0., xInit),    " Z0 ", Z0," DERREAL IS  ", model.derSol(0., xInit), "Duration", t1 - t0) #, "Gamma0 " , Gamma0)
